...
import com.msb.bpm.approval.appr.model.response.usermanager.GetUserProfileResponse.User;
import com.msb.bpm.approval.appr.notication.LandingPageNotificationStrategy;
...
/*
* @author: BaoNV2
* @since: 6/9/2023 10:46 PM
* @description:  
* @update:
*
* */
@ExtendWith(MockitoExtension.class)
class CmsPushDocumentsServiceImplTest {
  ...
  public void setup() {
    ...
    request = JsonUtil.convertString2Object(
        "{\"actionType\":\"New\",\"confirmStatus\":\"REJECT\",\"reason\":\"\",\"documents\":[{\"cmsDomainReferenceId\":\"bd109876ee-02\",\"docCode\":\"0101\",\"docName\":\"CMND/CCCD/Hộ chiếu\",\"group\":\"10\",\"files\":[{\"minioPath\":\"bpm-sit/mft/cjbo/20230823/151-00006116/hoSo3.pdf\",\"status\":\"SUCCESS\",\"fileName\":\"hoSo3.pdf\"}],\"metadata\":null}]}",
  ...
  @Test
  void test_get_type_successful() {
    Assert.assertEquals(CMS_PUSH_DOCUMENTS, cmsPushDocumentsService.getType());
  }

  @Test
  void test_execute_successful() throws ApiException {
    ExternalTaskSubscription subscription = new ExternalTaskSubscription();
    subscription.setProcessDefinitionKey("key");
    subscription.setIncludeExtensionProperties(true);
    Map<String, ExternalTaskSubscription> subscriptions = new HashMap<>();
    subscriptions.put(CHECK_WORKFLOW.name(), subscription);
    ProcessInstanceWithVariablesDto renturnVar = new ProcessInstanceWithVariablesDto();

    entityApp.setLdpStatus(null);
    ...
    entityApp.setCustomer(customerEntity);

//    when(camundaProperties.getSubscriptions()).thenReturn(subscriptions);
    when(commonService.findAppByBpmId(any())).thenReturn(entityApp);
//    when(camundaService.startProcessInstance(any(),any(),any())).thenReturn(renturnVar);
    when(applicationRepository.saveAndFlush(any())).thenReturn(null);
    when(factory.getNotification(NoticeType.LDP)).thenReturn(ldpNotificationStrategy);
    when(commonService.getUserDetail(any())).thenReturn(getUserProfileResponse);
    Assertions.assertDoesNotThrow(() ->
        cmsPushDocumentsService.execute(request, "151-00006116"));
  }

  @Test
  void test_saveChecklist_succesful() {
    Map<String, Object> mapFileSize = new HashMap<>();
    List<RuleVersionMappingEntity> ruleVersionMapping = new ArrayList<>();
    ...
    when(ruleVersionMappingRepository.findByApplicationIdAndRuleCode(any(),any())).thenReturn(Optional.of(ruleVersionMapping));
    when(checklistService.getChecklistByRequestCode(any())).thenReturn(checklistBaseResponse);
    when(cmsBaseIntegrationService.setAdditionIdAndChecklistMappingId(any(),any(),any(),any())).thenReturn(true);
    when(minioProperties.getMinio()).thenReturn(minio);
    Assertions.assertDoesNotThrow(() -> cmsPushDocumentsService.saveChecklist(entityApp, request, mapFileSize));
  }
