...
public class TeamLeadAssignContactSenderStrategy extends AbstractBaseService implements SenderStrategy {
  ...
  public EventCodeEmailType getType() {
    return TEAM_LEAD_ASSIGN_CONTACT;
  }
  ...
  private void sendMail(ApplicationEntity entityApp, String eventCode){
    ...
    params.put(ParamsEmailCommon.ASSIGNEE, metadata.get(ASSIGNEE));
    ...
    EmailRequest dataSend = EmailRequest.builder()
            .to(Arrays.asList(metadata.get(ASSIGNEE) + EMAIL_ADDRESS_MSB))
            .cc(lstEmailCc)
            .eventCode(TEAM_LEAD_ASSIGN_CONTACT.name())
            .params(params)
            .build();
